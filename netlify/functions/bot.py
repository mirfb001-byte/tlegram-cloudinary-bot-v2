...
def handler(event, context):
    ...
    if event.get('httpMethod') == 'POST':
        try:
            ...
            if 'message' in body and ('photo' in body['message'] or 'video' in body['message']):
                ...
                # 3. Get file path from Telegram's API
                ...
                if 'result' in file_info and 'file_path' in file_info['result']:
                    ...
                    # 4. Upload to Cloudinary directly using the Telegram URL stream
                    upload_result = cloudinary.uploader.upload(download_url, resource_type=resource_type)
                    cloudinary_url = upload_result.get("secure_url")

                    # 5. Build response and reply back to the user
                    ...
                    # Target endpoint (sendPhoto or sendVideo)
    ...
    # Netlify functions must always return a 200 OK status code back to Telegram
    return {
        'statusCode': 200,
